      ...
      {region.kind === "text" ? (
        ...
        <>
          ...
          <div className={styles.mediaChoices}>
            {media.map((asset) => <button type="button" key={asset.id} onClick={() => onChoose(asset)}><Image unoptimized width={320} height={240} src={asset.url} alt="" /><span>{asset.label}</span></button>)}
          </div>
        </>
      )}
